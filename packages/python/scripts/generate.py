...
def generate(destination: Path) -> None:
    ...
    for version in sorted(path for path in SOURCE_SCHEMAS.iterdir() if path.is_dir()):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for source in generated.rglob("*.py"):
                ...
                if target.exists():
                    raise SystemExit(f"generated model collision: {target.relative_to(destination)}")
